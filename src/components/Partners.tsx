      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 items-center justify-items-center">
          {partners.map((partner) => (
            <div
              key={partner.id}
              className="bg-white/10 p-6 rounded-lg hover:bg-white/20 transition-colors duration-300 flex flex-col items-center justify-center h-48 w-full"
            >
              <img
                src={partner.image}
                alt={partner.name}
                className="w-[200px] h-[100px] object-cover rounded-xl shadow-[0_0_20px_rgba(255,255,200,0.4)]"
              />
              <p className="mt-4 text-sm text-white/80 text-center">{partner.name}</p>
            </div>
          ))}
        </div>
      </div>
